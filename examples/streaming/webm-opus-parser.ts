...
function decodeElementValue(
	...
	if (info.type === "u") {
		return { name: info.name, type: info.type, value: readUnsigned(data), data };
	}
	if (info.type === "s") {
		return {
			name: info.name,
...
function drainCompletedMasters(
	...
	while (state.elementStack.length > 0) {
		...
		elements.push({ name: top.name, type: "m", isEnd: true, data: EMPTY_BYTES });
	}
}

function parseBlockFrames(payload: Uint8Array, lacing: number): Uint8Array<ArrayBufferLike>[] {
	if (lacing === 0) {
	...
	if (lacing === 1) {
		...
		for (let index = 1; index < frameCount - 1; index++) {
			...
			if (!delta) return [];
			cursor += delta.length;
			const nextSize = sizes[index - 1]! + delta.value;
			sizes.push(nextSize);
			total += nextSize;
		}
		sizes.push(payload.length - cursor - total);
...
function parseWebmElements(
	...
	while (true) {
		...
		const info = WEBM_ELEMENTS.get(id.value) ?? { name: "unknown", type: "b" as const };
		const dataSize = size.value;
		const absoluteContentStart = state.byteOffset + cursor + headerSize;
		cursor += headerSize;

		if (info.type === "m") {
			elements.push({ name: info.name, type: "m", isEnd: false, data: EMPTY_BYTES });
			...
			if (!element.isEnd) {
				...
				if (element.name === "TrackEntry") {
					if (
						...
						head = codecPrivate != null
							? parseOpusHead(codecPrivate) ?? createFallbackOpusHead(channels)
			...
			const baseTimestampUs = Math.max(
				...
				Math.round(
					((state.clusterTimestamp + block.timecode) * state.timestampScaleNs -
						state.activeTrack.codecDelayNs) / 1_000,
				),
			);
			for (let index = 0; index < block.frames.length; index++) {
				packets.push({
					packet: block.frames[index]!,
