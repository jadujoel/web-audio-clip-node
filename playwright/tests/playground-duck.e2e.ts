...
test.describe("Playground ducking", () => {
	...
	test("ducking produces smooth gain transitions without clicks", async ({
		page,
		browserName,
	}) => {
		test.skip(
		...
		// Enable ducking with moderate settings (soft knee should prevent clicks)
		const duckControl = page.locator(
			"fieldset:has(legend:has-text('Sidechain Duck'))",
		);
		const duckToggle = duckControl.locator("input[type='checkbox']");
		...
		// Start the kick — the RMS-based detection + soft knee + hold time should
		// produce smooth transitions rather than harsh clicks
		await page.locator("button:has-text('Start Kick')").click();
		await page.waitForTimeout(2000);

		// Measure several intervals of audio to verify gain transitions are smooth
		// (no extreme jumps between consecutive measurements)
		const samples = await measureAudioSustain(page, 3000, 25, 0.001);

		// Check that consecutive RMS values don't have extreme jumps
		// (a sign of clicking / discontinuities at duck onset).
		// With soft knee + SR-aware smoothing, transitions should be gradual.
		let maxJump = 0;
		for (let i = 1; i < samples.rmsValues.length; i++) {
			const jump = Math.abs(samples.rmsValues[i]! - samples.rmsValues[i - 1]!);
			if (jump > maxJump) maxJump = jump;
		}

		// The max jump between consecutive 25ms RMS measurements should be modest.
		// With the old algorithm (peak detection, no soft knee), jumps could be very large.
		// With the new algorithm, transitions are smooth enough that jumps stay below 0.3.
		expect(maxJump).toBeLessThan(0.3);

		// Audio should still be present (ducking doesn't kill it entirely)
		expect(samples.activeSamples).toBeGreaterThan(0);
	});
});
