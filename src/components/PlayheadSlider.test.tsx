...
describe("PlayheadSlider", () => {
	...
	test("does not render streaming overlays", () => {
		render(<PlayheadSlider value={0} audioDuration={8} onChange={noop} />);
		expect(screen.queryByTestId("streaming-playhead-track")).toBeNull();
		expect(screen.queryByTestId("playhead-buffer-track")).toBeNull();
	});
});
