...
export function App() {
	...
	useEffect(() => {
		...
		const tick = () => {
			...
			if (f) {
				...
				setValues((prev) =>
					prev.playhead === ph ? prev : { ...prev, playhead: ph },
				);
			}
	...
	const handleToggle = useCallback((key: string, on: boolean) => {
		setEnabled((prev) => ({ ...prev, [key]: on }));
		const node = nodeRef.current;
		if (node) applyToggle(node, key, on);
	}, []);

	const handleSnapChange = useCallback((key: string, snap: string) => {
	...
	return (
		<main>
			...
			<section id="buttons">
				...
				<button type="button" onClick={handleDispose}>
					Dispose
				</button>
				<button type="button" onClick={handleLog}>
					Log State
				</button>
				<button type="button" onClick={handleLoadSound}>
					Load Sound
				</button>
			</section>
			<hr />
